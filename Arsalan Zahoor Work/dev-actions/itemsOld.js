console.log('In Old items Action');
exports.items = {
    name: 'itemsOld',
    ...
    run: function(api, connection, next){
        console.log('filters',connection.params.filters,typeof connection.params.filters,connection.params.filters.length);
        var request=require('request');
        var trim = require('trim');
        var body;
        if(connection.params.filters.length <= 3){
            body = {
                "from" : 0, 
                "size" : 50,
                "query": {
  ...
  "aggs": {
    ...
    "suppliers": {
      "terms": {
        ...
        request({
            method: 'POST',
            url:'http://192.168.1.41:9200/item_balance/_search?from=0&size=50',
            body:body,
            json:true
