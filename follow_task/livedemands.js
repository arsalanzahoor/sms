console.log("live demands testing");
...
var q = async.queue(function (task, callbackq) {
    ...
    async.waterfall([
        ...
        function(res1, callback_t) {
            ...
            var sqlqry = 'select a.customer,a.mobile,a.itemdemandsid as demandid from '+channel+'.itemdemands as a join '+channel+'.itemdemanddetails b on a.itemdemandsid = b.fkdemandid where a.status=1 and a.sms_status=0 and b.fkbarcodeid='+task.data.itemid+' group by a.mobile,b.fkbarcodeid';
        ...
        function(result, callback_u) {
            ...
            if(result.length===0){
                ...
                var sqlquery = 'update '+channel+'.itemdemands set status=2 where  itemdemandsid in ('+demandsbycommas+')';
...
var q2 = async.queue(function(task, callback) {
    ...
    request({
        method: 'POST',
        url:'http://kohsar.esajee.com/admin/api.php?method=smsLog&type=post&user_id=1888', 
        form:data
