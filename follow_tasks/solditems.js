console.log('In solditems Action');
exports.categories = {
    name: 'solditems',
    description: 'search solditems from elastic search',
    inputs: {
        stock : {
            required : true
        }
    ...
    run: function(api, connection, next){
        ...
        request({
            method: 'POST',
            url:'http://192.168.1.41:9200/item_balance/_search?size=700',
            body:body,
            json:true
            ...
            if (!error && response.statusCode == 200) {
                ...
                for(var i = 0;i < result.hits.hits.length;i++){
                    ...
                    if(result.hits.hits[i]._source.data.item_img != null && result.hits.hits[i]._source.data.item_img != 'http://www.esajeecom.esajee.com/media/catalog/product/cache/1/thumbnail/9df78eab33525d08d6e5fb8d27136e95/images/catalog/product/placeholder/thumbnail.jpg'){
                        arr.push({
                            'itemName':result.hits.hits[i]._source.data.item_description,
                            'itemImage':result.hits.hits[i]._source.data.item_img,
